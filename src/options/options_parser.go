...
var cliArguments struct {
	...
	UseCaching bool `long:"use-caching" description:"Set whether or not to use cache."`;
	CacheDuration int `long:"cache-duration" description:"Sets the cache duration(unit: min, 1~60)." default:"10"`;
	
	UsePreloading bool `long:"use-cache" description:"Set whether or not to use cache."`;
...
func ParseArguments() (*options, error) {
	...
	if len(cliArguments.ConversionOptions) > 0 {
	...
	if cliArguments.UseCaching {
		if cliArguments.CacheDuration >= 1 && cliArguments.CacheDuration <= 60 {
			resultOptions.CacheDuration = cliArguments.CacheDuration;
		} else {
			resultError = errors.New("The cache duration is out of range.");
		}
	}
	...
	if cliArguments.UsePreloading && len(cliArguments.ItemsToPreload) > 0 {
		itemsToPreload := strings.Split(cliArguments.ItemsToPreload, "|");
